...
void squares(int size1, int x_offset, int y_offset, int size2) {
  ...
  for(int y = 0; y < h; y++){
    ...
    //print a newline
      int check1 = isInRange(x, x_offset, size2)&& isAtBorder(y, y_offset, size2 - 1);
      int check2 = isInRange(y, y_offset, size2)&& isAtBorder(x, x_offset, size2 - 1);
      if(check1 || check2){
	printf("*");
      } else{
	check1 = x < size1 && (y == 0 || y == size1 - 1);
	check2 = y < size1 && (x == 0 || x == size1 - 1);
	if(check1 || check2){
	  printf("#");
	} else{
	  printf(" ");
